<?php
	...
	function generateRow($from, $to, $conn){
		...
		while ($row = $result->fetch_assoc()) {
			$stmt_details = $conn->prepare("SELECT * FROM details LEFT JOIN products ON products.id=details.product_id WHERE sales_id=?");
			$stmt_details->bind_param('i', $row['salesid']);
	...
	if(isset($_POST['print'])){
	    ...
	    $pdf->SetFont('dejavusans', '', 11);  // Use DejaVuSans font for UTF-8 support
	    $pdf->AddPage();  
	    $content = '';  
	    $content .= '
